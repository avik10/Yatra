...
const CaptainRidingConfirmPanel = (props) => {
    ...
    return (
        <div>
            <h5 className='absolute w-full top-0 text-center font-light mb-2 ' onClick={(e) => { props.setRidePanel(false) }}> <i className="ri-arrow-down-wide-line text-4xl text-gray-400"></i></h5>
            ...
            <div className='flex flex-col justify-between items-center'>
                ...
                <form className='w-full' onSubmit={submitHandler}>
                    <input type="text" className='bg-[#eee] px-10 py-3 rounded-lg text-lg font-semibold w-full mt-3 font-mono' placeholder='OTP' name="otp" id="otp" value={otp} onChange={(e) => { setotp(e.target.value) }} />
                    <div className='w-full flex justify-between items-center  mt-5' >
                        <Link to={'/captain-riding'} className='w-1/2 flex justify-center font-bold  bg-green-500 text-white rounded px-4 py-2 text-lg placeholder:text-base'>Confirm</Link>
                        <button onClick={(e) => { props.setRidePanel(false); props.setCaptainconfirmRidePanel(false) }} type='submit' className='w-1/2  font-bold  bg-red-500 text-white rounded px-4 py-2 text-lg placeholder:text-base'>Cancel</button>
                    </div>
                </form>
            </div>
        </div >
...
export default CaptainRidingConfirmPanel